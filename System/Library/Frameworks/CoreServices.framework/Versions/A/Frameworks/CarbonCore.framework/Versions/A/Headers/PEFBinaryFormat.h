/*
     ...
     Version:    CarbonCore-675.1.dallas~1
...
struct XLibContainerHeader {
  ...
  OSType              tag1;                   /* Must contain 'ðMac'.*/
  OSType              tag2;                   /* Must contain 'vLib' or 'bLib'.*/
  UInt32              currentFormat;          /* The version of XLib types used to create this container.*/
  UInt32              containerStringsOffset; /* Container offset of the container string table.*/
  UInt32              exportHashOffset;       /* Offset of the export hash table.*/
  UInt32              exportKeyOffset;        /* Offset of the export key table.*/
  UInt32              exportSymbolOffset;     /* Offset of the export symbol table.*/
  UInt32              exportNamesOffset;      /* Offset of the export names.*/
  UInt32              exportHashTablePower;   /* Export hash table size as log 2.  (Log2('h'))*/
  UInt32              exportedSymbolCount;    /* Number of exported symbols.  ('e')*/

                                              /* Fragment description fields:*/

  UInt32              fragNameOffset;         /* The offset of the fragment name in the container string table.*/
  UInt32              fragNameLength;         /* The length of the fragment name in the container string table.*/
  UInt32              dylibPathOffset;        /* The offset of the dyld "install name" in the container string table.*/
  UInt32              dylibPathLength;        /* The length of the dyld "install name" in the container string table.*/
  OSType              cpuFamily;              /* The ISA for code sections.  Constants in CodeFragments.h.*/
  OSType              cpuModel;               /* Specific CPU model if applicable.*/
  UInt32              dateTimeStamp;          /* Mac format creation stamp.*/
  UInt32              currentVersion;         /* Current version number for the code fragment.*/
  UInt32              oldDefVersion;          /* Old definition version number for the code fragment.*/
  UInt32              oldImpVersion;          /* Old implementation version number for the code fragment.*/
...
struct XLibExportedSymbol {
  UInt32              classAndName;           /* A combination of class and name offset.*/
  UInt32              bpOffset;               /* Container offset of the export's dyld binding pointer.*/
};
...
struct PEF2ContainerHeader {
                                              /* Structural fields:*/
  OSType              tag1;                   /* Must contain 'Joy!'.*/
  OSType              tag2;                   /* Must contain 'PEF '.*/
  UInt32              currentFormat;          /* The version of PEF2 used to create this container.*/
  UInt32              oldestFormat;           /* Oldest compatible container handler.*/
  UInt32              containerHeaderSize;    /* The size of this header in bytes.*/
  UInt32              containerLength;        /* The total length of the container in bytes.*/
  UInt32              checksum;               /* A checksum for the entire container.*/
  UInt32              sectionHeadersOffset;   /* Container offset of the first section header.*/
  UInt32              sectionHeaderSize;      /* The size in bytes of each section header.*/
  UInt32              totalSectionCount;      /* Total number of section headers.*/
  UInt32              instSectionCount;       /* Number of instantiated sections.*/
  UInt32              loaderSectionIndex;     /* Index of the section containing runtime loader tables.*/
  UInt32              containerStringsOffset; /* Container offset of the container string table.*/
  UInt32              containerStringsLength; /* Length in bytes of the container string table.*/
  UInt32              options;                /* Array of 32 option bits.*/
  UInt32              preferredAddress;       /* Preferred container address, 0xFFFFFFFF indicates no preference.*/
  UInt8               alignment;              /* Required container alignment as LOG 2.*/
  UInt8               stringEncoding;         /* The encoding for all strings in the container.*/
  UInt16              reservedA;              /* Reserved, must be written as zero.*/
  UInt32              reservedB;              /* Reserved, must be written as zero.*/
  UInt32              reservedC;              /* Reserved, must be written as zero.*/
                                              /* Fragment description fields:*/
  UInt32              nameOffset;             /* The offset of the name in the container string table.*/
  OSType              architecture;           /* The ISA for code sections.  Constants in CodeFragments.h.*/
  UInt32              dateTimeStamp;          /* Macintosh format creation/modification stamp.*/
  UInt32              currentVersion;         /* Current version number for the code fragment.*/
  UInt32              oldDefVersion;          /* Old definition version number for the code fragment.*/
  UInt32              oldImpVersion;          /* Old implementation version number for the code fragment.*/
  UInt32              reservedD;              /* Reserved, must be written as zero.*/
  UInt32              reservedE;              /* Reserved, must be written as zero.*/
};
...
struct PEF2SectionHeader {
  UInt32              nameOffset;             /* Offset of the name within the container string table.*/
  UInt32              presumedAddress;        /* Presumed address, affects relocations.*/
  UInt32              totalLength;            /* Fully expanded size in bytes of the section contents.*/
  UInt32              unpackedLength;         /* Size in bytes of the "initialized" part of the contents.*/
  UInt32              containerLength;        /* Size in bytes of the raw data in the container.*/
  UInt32              containerOffset;        /* Offset of section's raw data within the container.*/
  UInt32              options;                /* Array of 32 option bits.*/
  UInt8               shareKind;              /* Sharing level, if a writeable section.*/
  UInt8               alignment;              /* Required alignment, expressed as log 2.*/
  UInt16              reservedA;              /* Reserved, must be written as zero.*/
  UInt32              reservedB;              /* Reserved, must be written as zero.*/
  UInt32              reservedC;              /* Reserved, must be written as zero.*/
};
...
enum {
  ...
  kPEF2SectionHasCodeMask       = 0x00000001, /* Affects cache flushing operations.*/
  kPEF2SectionIsWriteableMask   = 0x00000002, /* Affects MMU access.*/
  kPEF2SectionHasRelocationsMask = 0x00000004, /* The section has runtime relocations.*/
  kPEF2SectionContentsArePackedMask = 0x00000100, /* The raw data is compressed.*/
  kPEF2SectionNoZeroFillMask    = 0x00000200, /* "Uninit" part is not zero filled.*/
  kPEF2SectionResidentMask      = 0x00000400, /* The section should be RAM resident.*/
                                        /* Bits that describe higher level semantics.*/
  kPEF2SectionFollowsPriorMask  = 0x00010000, /* Raw data is related to prior section.*/
  kPEF2SectionPrecedesNextMask  = 0x00020000, /* Raw data is related to next section.*/
...
struct PEF2LoaderInfoHeader {
  UInt32              headerSize;             /* Size in bytes of the loader info header.*/
  UInt32              options;                /* An array of 32 option bits.*/
  SInt32              mainSection;            /* Section containing the main symbol, -1 => none.*/
  UInt32              mainOffset;             /* Offset of main symbol.*/
  SInt32              initSection;            /* Section containing the init routine's TVector, -1 => none.*/
  UInt32              initOffset;             /* Offset of the init routine's TVector.*/
  SInt32              termSection;            /* Section containing the term routine's TVector, -1 => none.*/
  UInt32              termOffset;             /* Offset of the term routine's TVector.*/
  SInt32              notifySection;          /* Section containing the notification routine's TVector, -1 => none.*/
  UInt32              notifyOffset;           /* Offset of the notification routine's TVector.*/
  UInt32              importedLibrariesOffset; /* Offset of the imported library table.*/
  UInt32              importedLibrarySize;    /* The size in bytes of an imported library entry.*/
  UInt32              importedLibraryCount;   /* Number of imported libraries. ('l')*/
  UInt32              importedSymbolsOffset;  /* Offset of the imported symbol table.*/
  UInt32              importedSymbolSize;     /* The size in bytes of an imported symbol entry.*/
  UInt32              totalImportedSymbolCount; /* Total number of imported symbols.  ('s')*/
  UInt32              loaderNamesOffset;      /* Offset of the loader name table.*/
  UInt32              loaderNamesLength;      /* Total number of bytes in the loader name table.*/
  UInt32              exportHashTableOffset;  /* Offset of the export hash slot table.*/
  UInt8               exportHashTablePower;   /* Hash slot count as log 2.*/
  UInt8               reservedA;              /* Reserved, must be zero.*/
  UInt16              reservedB;              /* Reserved, must be zero.*/
  UInt32              exportedKeysOffset;     /* Offset of the exported symbol key table.*/
  UInt32              exportedSymbolsOffset;  /* Offset of the exported symbol table.*/
  UInt32              exportedSymbolSize;     /* The size in bytes of an exported symbol entry.*/
  UInt32              exportedSymbolCount;    /* Number of exported symbols. ('e')*/
  UInt32              relocHeadersOffset;     /* Offset of the relocation headers.*/
  UInt32              relocHeaderCount;       /* Number of sections with relocations.*/
  UInt32              relocInstrOffset;       /* Offset of the relocation instructions.*/
  UInt32              relocInstrLength;       /* Total number of bytes of relocation instructions.*/
  UInt32              reservedC;              /* Reserved, must be zero.*/
  UInt32              reservedD;              /* Reserved, must be zero.*/
};
...
struct PEF2ImportedLibrary {
  UInt32              nameOffset;             /* Imported name table offset of library's name.*/
  UInt32              oldImpVersion;          /* Oldest compatible implementation version.*/
  UInt32              currentVersion;         /* Current version at build time.*/
  UInt32              importedSymbolCount;    /* Imported symbol count for this library.*/
  UInt32              firstImportedSymbol;    /* Index of first imported symbol from this library.*/
  UInt32              options;                /* Option bits for this library.*/
...
struct PEF2LoaderRelocationHeader {
  UInt32              sectionIndex;           /* Index of the section to be fixed up.*/
  UInt32              relocLength;            /* Number of bytes of relocation items.*/
  UInt32              firstRelocOffset;       /* Byte offset of first relocation instruction.*/
  UInt32              reservedA;              /* Reserved, must be zero.*/
};
